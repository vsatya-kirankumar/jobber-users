import { IOrderMessage, IRatingTypes, IReviewMessageDetails, ISellerDocument } from '@vsatya-kirankumar/jobber-shared';
...
const getRandomSellers = async (count: number): Promise<ISellerDocument[]> => {
  ...
  const sellers: ISellerDocument[] = await SellerModel.aggregate(pipeline);
